...
public class VendorControllerTest {
	...
	public void vendorListVendorControllerTest() throws Exception {
		...
		/*
		 * VendorBean bean = new VendorBean("V001", "Only Vimal",
		 * "Stock Home Road,Sector 22, New Delhi-110 001", "John", "9002348970");
		 * List<VendorBean> vendorBeans = new ArrayList<>(); vendorBeans.add(bean);
		 * 
		 ...
		 * 
		 * // check size of json object .andExpect(jsonPath("$", hasSize(1)))
		 * 
		 ...
		 * 
		 * //check the return value .andExpect(jsonPath("$[0].vendorId").value("V001"))
		 * .andExpect(jsonPath("$[0].vendorName").value("Only Vimal"))
		 * .andExpect(jsonPath("$[0].vendorAddress").
		 * value("Stock Home Road,Sector 22, New Delhi-110 001"))
